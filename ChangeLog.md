...
The cosponsored legislation counts now exclude withdrawn cosponsorships. 
## Change #2 
...
Error messages in the Bill text-level endpoint, the Bill summary-level endpoint, and the Amendment text-level endpoint were investigated and fixed. This is related to [Issue #431](https://github.com/LibraryOfCongress/api.congress.gov/issues/431)and [Issue #434](https://github.com/LibraryOfCongress/api.congress.gov/issues/434). 
## Change #3
### Impacted endpoints: /amendment, /amendment/{congress}/{amendmentType}
The amendment list-level endpoints were fixed so that the default sort is by latest action date. This is related to [Issue #406](https://github.com/LibraryOfCongress/api.congress.gov/issues/406). **This work is ongoing from a previous sprint.**
...
A referrer URL to the committee's official website was added to this endpoint. 
## Change #2
### Impacted endpoint: /committee-meeting/{congress}/{chamber}/{eventId}
The continuation date and time wwas modified to the following format: YYYY-MM-DDT00:00:00Z. The `<meeting_dt>` element was renamed `<continuationDate>`.
...
The amendment list-level endpoint was modified so the default sort is by amendment ID with an optional sort by `<updateDate>`. This was related to [Issue #228](https://github.com/LibraryOfCongress/api.congress.gov/issues/228). 
## Change #3
### Impacted endpoint: /bill/{congress}/{billType}/{billNumber} 
A `<legislationUrl>` element was added to the bill item-level endpoint. This will link to the bill's information on Congress.gov. This was related to [Issue #268](https://github.com/LibraryOfCongress/api.congress.gov/issues/268).
# COMPLETED August 2025, Part 1 | [Milestone](https://github.com/LibraryOfCongress/api.congress.gov/milestone/49)
## Change #1
### Impacted endpoint: /committee-meeting
An error message in the commmittee meeting endpoint was investigated and fixed. This was related to [Issue #253](https://github.com/LibraryOfCongress/api.congress.gov/issues/253).
## Change #2
### Impacted endpoint: /crsreport/{reportNumber}
A bug related to CRS report versions updating was investigated and fixed. This was related to [Issue #346](https://github.com/LibraryOfCongress/api.congress.gov/issues/346).
## Change #3
### Impacted endpoint: /bill/{congress}/{billType}/{billNumber}
On behalf of sponsor data was incorporated into the Congress.gov API bill endpoint where applicable.
## Change #4
### Impacted endpoint: /crsreport
An `<updateDate> optional filter was added to the CRS report list-level endpoint. This was related to [Issue #328](https://github.com/LibraryOfCongress/api.congress.gov/issues/328).
## Change #5
### Impacted endpoint: /law/{congress}/{lawType}/{lawNumber}
An error message in the law detail endpoint was investigated and fixed. This was related to [Issue #235](https://github.com/LibraryOfCongress/api.congress.gov/issues/235).
## Change #6
...
# COMPLETED July 2025 | [Milestone](https://github.com/LibraryOfCongress/api.congress.gov/milestone/48)
## Change #1
...
There was a discrepancy between the web and the API for committee reports. The web app did not display reports for which Congress.gov has not yet received text and metadata from GPO, but the API did. This was addressed and fixed.
## Change #2
### Impacted endpoint: All committee endpoints 
When calling the /committee endpoint,  a 500 error response when using a `fromDateTime` parameter with certain dates was passed. This was addressed and fixed. This is related to [Issue #290](https://github.com/LibraryOfCongress/api.congress.gov/issues/290).
## Change #3
### Impacted endpoint: All CRS report endpoints
The `<publishDate>` element was adjusted so that it accurately displays the publish date of current version of the report. This is related to [Issue #322](https://github.com/LibraryOfCongress/api.congress.gov/issues/322).
## Change #4
### Impacted endpoint: All CRS Report endpoints
An error message `"error": "get() returned more than one CrsProductsProd -- it returned 2!` was investigated and fixed. This is related to [Issue #324](https://github.com/LibraryOfCongress/api.congress.gov/issues/324).
